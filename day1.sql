...
-- Databases: column definitions: VARCHAR(Similar to Java String), INT/ INTEGER(Java: int)
-- DECIMAL(13,2) 11 digits for integers, 2 digits for decimal places
...
-- INSERT DATA
-- SYNTAX: INSERT INTO "TSBLE_NAME" VALUES
...
SELECT * FROM STAFFS WHERE DEPT_CDOE = 'IT' AND GENDER = 'M';
-- SUPPORT BRACKETS
...
SELECT STAFF_NAME AS NAME, DEPT_CODE AS DEPARTMENT-CODE FROM STAFFS WHERE GENDER = 'M';
...
FROM STAFF S
WHERE S.DEPT_CODE = 'IT'; -- DETERMIN THE NUMBER OF ROWS
...
SELECT *
FROM STAFFS
WHERE EMPLOYMENT_DATE BETWEEN STR_TO_DATE('2015-01-01', '%Y-%m-%d') AND STR_TO_DATE('2015-12-31', '%Y-%m-%d');
...
-- 1. EXCLUDE THE EMPLOYEE IN MARKETING (WHERE IS THE ACTION BEFOR GROUP BY)
-- 2. GROUP BY
